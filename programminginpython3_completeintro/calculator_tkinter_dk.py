"""calculator_tkinter_dk.py 
reference link1: https://www.geeksforgeeks.org/python-simple-gui-calculator-using-tkinter/"""
from tkinter import *
expression = ""  
def press(num): 
	global expression 
...
def equalpress(): 
	try: 
		...
		equation.set(" Error! Please check your input being logged") 
		expression = "" 
def clear(): 
	global expression 
	expression = "" 
...
if __name__ == "__main__": 
	...
	gui.title("Calculator_TKinter_DK") 
	gui.geometry("270x150") 
	equation = StringVar() 
	expression_field = Entry(gui, textvariable=equation) 
	expression_field.grid(columnspan=4, ipadx=70) 
	equation.set('Enter your number/expression') 
	...
	myDecimal1= Button(gui, text='%', fg='black', bg='red', command=lambda: press('%'), height=1, width=7) #Return the remainder 
	myDecimal1.grid(row=6, column=1)
	myExpression2= Button(gui, text='TBD', fg='black', bg='red', command=lambda: press('^'), height=1, width=7) 
	myExpression2.grid(row=6, column=2)
	myExpression3= Button(gui, text='TBD', fg='black', bg='red', command=lambda: press('&'), height=1, width=7) 
	myExpression3.grid(row=6, column=3)
